        ...
        return [
            {
                ...
                delta: {label: "DB", tone: "neutral" as const}
            },
            {
                ...
                delta: {label: "users", tone: "neutral" as const}
            },
            {
                ...
                delta: {label: "hôm nay", tone: "neutral" as const}
            }
        ];
    }, [users.length]);
        ...
        <div className={styles.page}>
                        ...
                        onClick={() => {
                            exportExcelHtml({
                                ...
                                meta: {
                                    ...
                                    "Phòng ban": deptOptions.find((d) => d.id === deptFilter)?.label ?? "Tất cả",
                                },
                                columns: [
                                    {key: "code", label: "Mã NV", widthPx: 110, align: "center"},
                                    {key: "name", label: "Họ tên", widthPx: 220, align: "left"},
                                    {key: "department", label: "Phòng ban", widthPx: 180, align: "left"},
                                    {key: "role", label: "Chức vụ", widthPx: 150, align: "left"},
                                    {key: "status", label: "Trạng thái", widthPx: 130, align: "center"},
                                    {key: "email", label: "Email", widthPx: 220, align: "left"},
                                ],
                                rows: filtered.map((u) => ({
                                    code: u.code || `#${u.id}`,
                                    name: u.name,
                                    department: u.department_id
                                        ? deptById.get(u.department_id)?.name ?? `#${u.department_id}`
                                        : "—",
                                    role: u.role || "—",
                                    status: viStatusLabel(u.status),
                                    email: u.email || "—",
                                })),
                            });
                        }}
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={styles.empStats}>
                                    ...
                                    <div className={styles.empStatItem}>
                                        <div
                                            className={styles.empStatValSmall}>{u.email ? u.email.split("@")[0] : "—"}</div>
                                        <div className={styles.empStatLbl}>Email</div>
                                    </div>
                                    <div className={styles.empStatItem}>
                                        <div
                                            className={u.status === "active" ? `${styles.empStatVal} ${styles.ok}` : `${styles.empStatVal} ${styles.warn}`}>{viStatusLabel(u.status)}</div>
                ...
                >
                    <Table>
                      ...
                      <span className={styles.empSub}>{u.code || `#${u.id}`}</span>
                                ...
                                <td>
                                    <span
                                        className={u.status === "active" ? `${styles.tag} ${styles.good}` : `${styles.tag} ${styles.bad}`}>{viStatusLabel(u.status)}</span>
                                </td>
                                ...
                                <td>
                                    <div className={styles.rowActions}>
                                        ...
                                        <Tooltip title={u.status === "active" ? "Tạm tắt" : "Kích hoạt"}
                                                 placement="top">
                                            <button
                    ...
                    {filtered.length === 0 ?
                        <div className={styles.empty}>Chưa có nhân viên (hoặc không khớp tìm kiếm).</div> : null}
                </Card>
            )}
            ...
            <div className={styles.pagination}>
                ...
                <div className={styles.pageControls}>
                    <button className={styles.pageBtn} type="button" onClick={() => setPage((p) => Math.max(1, p - 1))}
                            disabled={pageSafe <= 1}>
                        <LeftOutlined/>
                    </button>
                    {pageButtons.map((p) => (
            ...
            {editing ? <EditOutlined/> : <PlusOutlined/>}
                ...
                footer={
                    <>
                        <button className={styles.btnGhost} type="button" onClick={() => setModalOpen(false)}
                                disabled={loading}>
                            Hủy
                        </button>
                        <button
            ...
            >
                ...
                <div className={styles.modalGrid}>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <input className={styles.input} value={email} onChange={(e) => setEmail(e.target.value)}
                               placeholder="VD: a@company.vn"/>
                        {!editing ?
                            <div className={styles.fieldHint}>Bắt buộc để gửi link kích hoạt tài khoản.</div> : null}
                    </div>

                    <div className={styles.formGroup}>
                        <div className={styles.formLabelTop}>Phòng ban</div>
                        <select className={styles.input} value={departmentId}
                    ...
                    {!editing ? (
                        <div className={styles.formGroup}>
                            ...
                            <select className={styles.input} value={portalRoleKey}
                                    onChange={(e) => setPortalRoleKey(e.target.value as any)}>
                                <option value="employee">Nhân viên</option>
                                <option value="manager">Quản lý</option>
                            </select>
                            <div className={styles.fieldHint}>Quản lý có thể xem/duyệt theo phạm vi công ty (không có
                                quyền IAM/Công ty).
                            </div>
                        </div>
                    ) : null}
                ...
                <div className={styles.modalNote}>Lưu ý: `code`/`email` là duy nhất trong phạm vi công ty. Nếu trùng sẽ
                    báo lỗi từ backend.
                </div>
            </Modal>
            ...
            <CameraOutlined/>
                ...
                footer={
                    <>
                        ...
                        {!cam.state.ready ? (
                            <button className={styles.btnGhost} type="button" onClick={() => cam.start()}
                                    disabled={loading}>
                                <CameraOutlined/> Bật camera
                            </button>
                        ) : (
                            <button className={styles.btnGhost} type="button" onClick={() => cam.switchCamera()}
                                    disabled={loading}>
            ...
            >
                ...
                <div className={styles.faceVideoWrap}>
                    ...
                    {!cam.state.ready ?
                        <div className={styles.facePlaceholder}>Bật camera để chụp ảnh khuôn mặt</div> : null}
                </div>
